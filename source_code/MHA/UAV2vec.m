function [data] = UAV2vec(img,obj,cam,params,data)
...
for k = 1 : search_num
        ...
        last_angle = data.last_angles(data.gopro_id);
%       search_angle = last_angle - pi/(search_num/2) * (search_num) + pi/(search_num/2) * (k);
        
        search_angle = last_angle - deg2rad(search_num/2) + deg2rad(k);
        
%         if k == 182
%             search_angle = last_angle - deg2rad(search_num/2) + deg2rad(k);
%         end 
        
        search_angle = mod(search_angle,2 * pi);        
        ...
        for i = 1 : obj_num  
            ...
            if left_ang > 2 * pi  % !todo : when the leftangle greater than 2*pi      
                ...
                angle_new(ang_gre) = angle(ang_gre) + 2 * pi;        
            end
           ...
           OV = [0,0,cos(vertical_ang),-sin(vertical_ang)];  
            ...
            if angle_new(i) > right_ang && angle_new(i) < left_ang     % !todo : when the leftangle greater than 2*pi
               flag(i) = 1;
